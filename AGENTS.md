...
- **Frontend**: Preact + TailwindCSS 4 + DaisyUI (Vite HMR in dev, CSR only — no SSR)
...
`deno fmt` (without `--check`) auto-fixes formatting issues. Always run `deno task check` after changes.
...
`DATABASE_DSN` is mandatory. If missing, the app throws on startup. SQLite fallback is planned for Docker but not implemented yet.
...
routes/             # Fresh 2 FS-routed handlers (routes/api/* + routes/v1/*)
  ...
  v1/               # OpenAI-compatible proxy — requires AUTH_KEY or proxy_auth_keys
...
jobs/               # Cron job stubs (not wired to Deno Cron yet)
```

### Auth Boundaries

- `/api/*` routes: `requireAdmin()` checks `Authorization: Bearer <AUTH_KEY>` only
- `/v1/*` routes: checks token against `AUTH_KEY` + `system_settings.proxy_auth_keys` (multi-line config)
